...
ErrorCode TiKVKeyValueIO::Get(const SizeType key,
                             std::string* value,
                             const std::chrono::microseconds& timeout,
                             std::vector<Helper::AsyncReadRequest>* /*reqs*/) {
    if (value == nullptr) return ErrorCode::Undefined;

    {
        std::lock_guard<std::mutex> g(m_mu);
        if (CacheGet(key, *value)) return ErrorCode::Success;
    }

    std::string v;
    int32_t st = m_client.GetU64((uint64_t)key, v, timeout);
    if (st != 0) return ErrorCode::Undefined;
...
ErrorCode TiKVKeyValueIO::MultiGet(const std::vector<SizeType>& keys,
                                  ...
                                  const std::chrono::microseconds& timeout,
                                  std::vector<Helper::AsyncReadRequest>* /*reqs*/) {
    if (values == nullptr) return ErrorCode::Undefined;
    values->clear();
    ...
    if (missKeys.empty()) return ErrorCode::Success;

    std::vector<std::string> missVals;
    int32_t st = m_client.MultiGetU64(missKeys, missVals, timeout);
    if (st != 0) return ErrorCode::Undefined;
    if (missVals.size() != missKeys.size()) return ErrorCode::Undefined;
...
ErrorCode TiKVKeyValueIO::Put(const SizeType key,
...
ErrorCode TiKVKeyValueIO::Merge(const SizeType key,
                               const std::string& value,
                               const std::chrono::microseconds& timeout,
                               std::vector<Helper::AsyncReadRequest>* reqs,
    ...
    if (checksum) {
        if (!checksum(oldv.data(), (int)oldv.size())) return ErrorCode::Undefined;
    }
