...
if [ "$PQ" -eq 1 ]; then
    ...
    # Default non-PQ run: Off + Pairs. Packed is strictly dominated by Pairs
    # without PQ (client-side full-precision rescoring is redundant); opt in
    # with --with-packed to restore the old 3-way A/B.
