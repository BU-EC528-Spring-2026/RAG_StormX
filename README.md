...
| [`results/benchmark_aerospike_udf_pairs_nopq.json`](results/benchmark_aerospike_udf_pairs_nopq.json) | **Filtered read** UDF only.                      | **Regression:** recall stays in line with the baseline (~0.84 by batch 9) but **mean search latency** rises to tens of ms (roughly an order of magnitude vs MultiGet), because the path issues **per-key UDF** work instead of a batched get. |
