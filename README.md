...
3. [Setup Guidance](#3-setup-guidance)
  ...
  - [Run TiKV Benchmark on Shared NVMe](#run-tikv-benchmark-on-shared-nvme)
...
- **Benchmarking:** We have been benchmarking SPTAG against **four storage backends** (FileIO, RocksDB, Aerospike, and TiKV) using the [SIFT1B (BigANN)](https://big-ann-benchmarks.com/) dataset on NVMe SSDs. Initial results show identical recall across backends under the same index/search settings, while latency/throughput differ by storage engine behavior and deployment topology. See the [Benchmarks](#4-benchmarks) section for full details.
...
### Run TiKV Benchmark on Shared NVMe
...
- `docker` and `docker compose` installed on the SPTAG node.
...
#### 1) Start Shared-NVMe TiKV Cluster
...
- PD: `127.0.0.1:2379,127.0.0.1:2479,127.0.0.1:2579`
- TiKV: `127.0.0.1:20160,127.0.0.1:20161,127.0.0.1:20162`

Quick health check (PD store list):

```bash
curl http://127.0.0.1:2379/pd/api/v1/stores
```

#### 2) Clean Perftest Cache Before Running

SPFresh benchmark code reuses cached `perftest_*` files if they already exist. If those files were generated by a different benchmark scale, they can poison the next run.
...
# Remove local benchmark cache files in current directory
rm -f perftest_vector.bin perftest_meta.bin perftest_metaidx.bin \
  perftest_addvector.bin perftest_addmeta.bin perftest_addmetaidx.bin \
  perftest_query.bin perftest_truth.* perftest_addtruth.* \
  perftest_batchtruth.* perftest_quanvectors.bin

# Remove stale cache accidentally created in repo parent directory
sudo rm -f ~/RAG_StormX/perftest_vector.bin ~/RAG_StormX/perftest_meta.bin ~/RAG_StormX/perftest_metaidx.bin \
       ~/RAG_StormX/perftest_addvector.bin ~/RAG_StormX/perftest_addmeta.bin ~/RAG_StormX/perftest_addmetaidx.bin \
       ~/RAG_StormX/perftest_query.bin ~/RAG_StormX/perftest_truth.* ~/RAG_StormX/perftest_addtruth.* \
       ~/RAG_StormX/perftest_batchtruth.* ~/RAG_StormX/perftest_quanvectors.bin
```

#### 3) Run TiKV Benchmark
...
This section walks through running the full SPTAG benchmark suite on the SIFT1B (BigANN) dataset across four storage backends: **FileIO**, **RocksDB**, **Aerospike**, and **TiKV**. It assumes you have already completed all the steps in [Setup Guidance](#3-setup-guidance) above.
...
If you have not already built the Docker image during the setup steps, build it now. The Dockerfile compiles SPTAG with RocksDB, Aerospike, and TiKV backends enabled (`-DROCKSDB=ON -DAEROSPIKE=ON -DTIKV=ON`).
...
#### If Your Local Repo Predates TiKV Integration

If your local clone was created before TiKV support was merged, update the repo and rebuild before running TiKV benchmarks.
...
# Regenerate kvproto gRPC/protobuf stubs used by TiKV integration
cd ThirdParty/kvproto
./generate_cpp.sh
cd ~/RAG_StormX/SPTAG

# Rebuild Docker image so /app/Release/SPTAGTest includes TiKV support
sudo docker build -t sptag .
```

Quick sanity check that TiKV support exists in the built test binary:

```bash
cd ~/RAG_StormX/SPTAG
strings Release/SPTAGTest | grep -E "TIKVIO|TiKVPDAddresses|ExtraDynamicSearcher:UseTiKV" | head
```
...
#### TiKV (Shared NVMe Local Cluster)

TiKV benchmark in this repo uses a local 3PD + 3TiKV cluster that shares one NVMe mount via per-node sub-directories.
