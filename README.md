...
> [!WARNING]
> **Register UDFs on the cluster first if you plan to run modes other than `Off`.** The SPTAG client no longer auto-uploads `sptag_posting.lua` on connect, so any run that exercises `Packed` (mode `1`) or `Pairs` (mode `2`) - including the **default** `run_aerospike_udf_ab.sh` sweep, which is `Off + Pairs` - will fail server-side until `avx_math.so` and `sptag_posting.lua` are registered. Follow [§6 Registering UDFs on remote Aerospike nodes](#6-registering-udfs-on-remote-aerospike-nodes) **before** the steps below. Mode `0` (Off) alone does not need any UDFs.
...
Search-path UDFs use two kinds of files under [`SPTAG/AnnService/udf/`](SPTAG/AnnService/udf/): the **Lua** module [`sptag_posting.lua`](SPTAG/AnnService/udf/sptag_posting.lua) (UDF entry points) and a **C** extension built from [`avx.math.c`](SPTAG/AnnService/udf/avx.math.c) into a shared library `avx_math.so` that Lua loads with `require "avx_math"`. The file [`test_avx_math_local.lua`](SPTAG/AnnService/udf/test_avx_math_local.lua) is a **client-side** smoke test only: it is not registered on Aerospike; use it to prove the compiled `.so` is safe before you register the module.
...
Run this on the SPTAG VM, or on whichever machine will run the `aql register module` commands:
...
> [!WARNING]
> `SPTAG_AEROSPIKE_HOST` must be an Aerospike node's **internal IP** that is reachable from the SPTAG VM/container. Do not use the SPTAG VM IP, an old cluster's IP, or a public/external IP unless you intentionally configured networking for that. Use this same IP for `aql`, the UDF benchmark, and the policy sweep.
...
### 4) Compile `avx.math.c` to `avx_math.so`

From the repo on the Aerospike node (or after copying just `SPTAG/AnnService/udf/` there):
...
### 5) Local smoke test **before** registering
...
```bash
aql -h "$SPTAG_AEROSPIKE_HOST"
aql> register module 'avx_math.so'
aql> register module 'sptag_posting.lua'
```

The interactive form only works when `aql` was started from a directory containing `avx_math.so` and `sptag_posting.lua`. If in doubt, use the absolute-path `-c` commands above.

The SPTAG benchmark script does not register these modules for you. Register them first, then run the benchmark.
...
If the probe returns `"avx_math:missing"` or the server log reports a load error, rebuild `avx_math.so` on an AVX-512 Aerospike node with the correct Lua headers and re-register both modules with `aql`.
